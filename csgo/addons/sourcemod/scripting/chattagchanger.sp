...
ConVar cv_old_tags, cv_new_tag, cv_colorful;
...
public void CVAR_Load(){
    cv_old_tags = CreateConVar("sm_chat_tag_changer_old_tags", "[SM],[oppa]", "8 tag 64 character (,)");
    cv_new_tag = CreateConVar("sm_chat_tag_changer_new_tags", "{0R}[csgo-turkiye.com],{0R}[CS:GO Türkiye]", "8 tag 64 character (,)\n{0R}=>random\n{01}=>white\n{02}=>darkred\n{03}=>purple\n{04}=>green\n{05}=>lightgreen\n{06}=>lime\n{07}=>red\n{08}=>grey\n{09}=>yellow\n{0A}=>bluegrey\n{0B}=>blue\n{0C}=>darkblue\n{0D}=>grey2\n{0E}=>orchid\n{0F}=>lightred\n{10}=>gold");
    cv_colorful = CreateConVar("sm_chat_tag_changer_colorful", "0", "Is the tag colorful? If active, the colors specified in the tag are removed.", _, true, 0.0, true, 1.0);
    AutoExecConfig(true, "chat_tag_changer", "CSGO_Turkiye");
    GetConVarString(cv_old_tags, s_old_tags, sizeof(s_old_tags));
    GetConVarString(cv_new_tag, s_new_tag, sizeof(s_new_tag));
    b_colorful = GetConVarBool(cv_colorful);
    GetTagSetting();
    HookConVarChange(cv_old_tags, OnCvarChanged);
    HookConVarChange(cv_new_tag, OnCvarChanged);
    HookConVarChange(cv_colorful, OnCvarChanged);
}

public int OnCvarChanged(Handle convar, const char[] oldVal, const char[] newVal)
{
    if(convar == cv_old_tags) strcopy(s_old_tags, sizeof(s_old_tags), newVal);
    else if(convar == cv_new_tag) strcopy(s_new_tag, sizeof(s_new_tag), newVal);
    else if(convar == cv_colorful) b_colorful = GetConVarBool(convar);
    GetTagSetting();
}

public void GetTagSetting(){
...
public Action Hook_TextMsg(UserMsg msg_id, Handle msg, const int[] players, int playersNum, bool reliable, bool init)
{
    if(reliable)
    {
        ...
        for (int i = 0; i < i_count; i++)
        {
            ...
            if( i_search >= 0 && i <= 5 ){
                Handle h_pack;
                CreateDataTimer(0.0, DataTimer, h_pack);
